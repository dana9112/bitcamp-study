...
public class Servlet01 extends HttpServlet {
  ...
  protected void service(HttpServletRequest request, HttpServletResponse response)
    ...
    // => 웹 브라우저는 세션 아이디를 보관해 두었다가 그 서버에 요청할 때 마다 세션 아이디를 보낸다.
    // 왜? 세셔 아이디는 쿠키이다.
    ...
    //
    // HTTP/1.1 200
    // Set-Cookie: JSESSIONID=C1A63C160D808BB74C25B87B034FA226; Path=/bitcamp-java-web; HttpOnly
    // Content-Type: text/plain;charset=UTF-8
    // Content-Length: 45
    // Date: Wed, 08 Apr 2020 03:11:25 GMT
    // Keep-Alive: timeout=20
    // Connection: keep-alive

    // (동영)
    // 새로고침해서 다시보면 set-Cookie 가 없을 것이다
    // 왜냐하면 브라우저가 이미 알고 있기 때문에 호출할 필요가 없다.
    // request Headers에 cookie로 주소가 넘겨져 있다.
