...
public class HelloController {
  ...
  // 클라이언트 요청이 들어왔을 때 호출될 메서드(request handler)를 표시하려면
  // 다음과 같이 @RequestMapping 애노테이션을 붙여야 한다.
  // => @RequestMapping(요청URL)
  // @RequestMapping("/hello")
  // @RequestMapping(value="/hello")
  // @RequestMapping(path="/hello")
  @RequestMapping({"/hello", "/hello2", "/okok"})
  // (동영) 웹페이지에서 출력을 보기 위해서는 ../app/hello 이렇게 주소를 넣어줘야 함

  // 리턴하는 String 값이 뷰 컴포넌트(예: JSP)의 URL이 아닌 경우
  // 애노테이션으로 표시한다.
  // => @ResponseBody : 리턴하는 문자열이 클라이언트에게 보낸 콘텐트임을 표시한다.
