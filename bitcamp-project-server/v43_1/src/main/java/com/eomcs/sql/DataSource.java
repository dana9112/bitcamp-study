...
public class DataSource {
  ...
  public Connection getConnection() throws Exception {
    ...
    if (conList.size() > 0 /* 객체가 있다는 것 */) {
      // 1) 있다면, 반납 받은 객체를 티너한다.리턴한다.
      ...
      con = new ConnectionProxy(DriverManager.getConnection(//
          ...
          username, //
          password));
      System.out.println("새 ConnectionProxy 객체 생성!");
    }

    // 물론 리턴하기 전에 스레드에Connection 객체의 보관한다.
  ...
  public void clean() {
    ...
    while (conList.size() > 0) {
      try {
        // 커넥션 리스트에 있는 모든 리스트를 꺼내고 닫고 꺼내고 닫고를 반복한다.
