...
public class MybatisDaoFactory {
  ...
  public <T> T createDao(Class<T> daoInterface) { // Object object ---- class<?> => 에서 제네릭 적용함
    return (T) Proxy.newProxyInstance(this.getClass().getClassLoader(), new Class[] {daoInterface},
        invocationHandler);
  }
}
// 클래스에다가 <> 를 적용하면 안된다.
// 왜냐하면 만약 lessonDao를 들어가면 lessonDao 전체가 다 적용되어야 하는데
// 우리는 넘어오는 파라미터 타입으로 값을 출력하기 때문에 클래스 전체에다가 제네릭을 적용하지 않는 이유이다.
// 반드시 리턴타입 앞에다가 제네릭을 적용해야 한다.
