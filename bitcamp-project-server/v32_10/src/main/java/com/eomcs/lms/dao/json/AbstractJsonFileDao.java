...
// 서브 클래스에게 공통 필드나 상속해주는 역할을 하기 때문에
// => abstract
...
public abstract class AbstractJsonFileDao<T> {
  ...
  protected void loadData() {
    ...
    try (BufferedReader in = new BufferedReader(new FileReader(file))) {
      ...
      T[] arr = (T[]) Array.newInstance((Class) itemType, 0); // 아이템이 원래 클래스다 (클래스로 형변환 해주자), T에 대해서
                                                              // 레슨, 보드에 대한 리스트를 각각 만들었음
