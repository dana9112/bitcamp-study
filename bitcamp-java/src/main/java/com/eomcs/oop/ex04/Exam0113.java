...
public class Exam0113 {
  ...
  public static void main(String[] args) throws Exception {
    ...
    byte[] bytes = {
        ...
        (byte)0x25, // 
        (byte)0x2b, // 
        (byte)0xea, (byte)0xb0, (byte)0xea,
        (byte)0xea, (byte)0xb0, (byte)0xea,
        (byte)0xeb, (byte)0x98, (byte)0xa,
    ...
    // 따라서 위 예제를 Linux, macOS, Unix 의 CLI(Command Line Interface; 콘솔창/명령창)에서 실행하면
    // 정상적으로 한글 문자열이 출력되지만, 
    ...
    // => 이클립스에서 JVM을 실행할 때 입출력 기본 인코딩을 UTF-8로 설정하기 때문이
    //
    // Windows CIL에서 깨지지 않게 하는 방법?
    // => JVM 실행 옵션에 다음을 추가하라
    //     -Dfile.encoding=UTF-8
    // => java -Dfile.encoding=UTF-8 -cp bin/main 클래스명
    // => PowerShell이 아닌 Command 창에서 실행하라!
    //    PoweShell에서는 -Dfile.encoding 옵션을 제대로 처리하지 못한다. 
    //    도트(.)를 분리 문자로 인식한다. 
