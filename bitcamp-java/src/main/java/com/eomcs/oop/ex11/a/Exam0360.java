...
public class Exam0360 {
  ...
  Runner createRunner(final String name) {
    ...
    class A implements Runner {
      ...
      /*
       * Exam0360 outer; // 바깥 클래스의 주소를 받을 필드 String name; // run() 메서드에서 사용할 로컬 변수의 값을 받을 필드
       *
       * String paramName;
       *
       * public A(Exam0365 obj, String str) { outer = obj}
       */

      @Override
      public void run() {
        // 바깥 메서드의 로컬 변수는 호출이 완료되는 순간
        // 스택 메모리에서 제거되기 때문에
        // 로컬 클래스의 객체에서 사용할 수 없는 상황이 발생할 것이다.
        // 그래서 컴파일러는 바깥 메서드의 로컬 변수 값을 저장할
        // 필드를 클래스에 추가한다.
        // 또한 로컬 변수의 값을 받는 생성자를 추가한다.
        ...
        System.out.printf("%s님이 달립니다!",
            name/* 로컬을 가르키는 것이 아니라 필드에 별도로 복사해둔 히든으로 만들어진 name (paranName) */);

        // 위 문장은 다음과 같이
        // 컴파일러가 로컬 클래스의 필드(예: paramName)를 사용하는 문장으로 바꾼다.
        // 그래서 creatorRunner() 메서드 호출이 끝나더라도
