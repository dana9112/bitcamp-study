// 인스턴스 초기화 블록(initializer block) - 생성자와 인스턴스 초기화 블록 (initializer block)
...
public class Exam0612 {
  static class A {
    ...
    A() {
      ...
      // 인스턴스 초기화 블록(initializer block)이 있으면 선언된 순서대로 생성자의 첫 부분에 복사한다.
      // 즉 위에 선언된 초기화 블록 안에 있는 코드가 그대로 다음과 같이 복사된다. 
      // JAVA Tutorial문서: Learning the Java Language > Classes and 
