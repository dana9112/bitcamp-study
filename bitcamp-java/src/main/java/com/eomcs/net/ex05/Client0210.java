...
public class Client0210 {
  public static void main(String[] args) throws Exception {
    ...
    // String message = "Hello"; //constatnt pool String 객체 생성
    byte[] bytes = "Hello".getBytes("UTF-8"); // hello는 String 객체이다.

    // 보낼 데이터를 패킷에 담는다.
    // => 패킷 = 데이터 + 받는이 주소 + 포트번호
    DatagramPacket packet = new DatagramPacket(//
        bytes, // 데이터가 저자된 바이트 배열
        bytes.length, // 전송할 데이터 개수
        InetAddress.getByName(receiver), // 데이터를 받을 상대편 주소 // socketAddress로 묶을 수 있다.
        port /* 포트번호 */);
