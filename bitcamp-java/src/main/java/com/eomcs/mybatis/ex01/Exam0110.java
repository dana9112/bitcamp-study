...
public class Exam0110 {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // 2. SqlSession Factory 만들어 줄 빌더 객체 준비
    SqlSessionFactoryBuilder factoryBilder = new SqlSessionFactoryBuilder();

    // 3. sqlSession 객체를 만들어 줄 팩토리 객체 준비
    // => mybatis는 Builder를 이용하여 SqlSessionFactory 객체를 만든다.
    // => 이때 공장 객체를 만들 때 사용할 설정파일을 지정한다.
    // => 설정 파일의 경로를 직접 지정하지 말고,
    // 해당 파일을 읽을 때 사용할 InputStrea을 넘겨줘라.
    //
    SqlSessionFactory factory = factoryBilder.build(mybatisConfigInputStream);

    // 4. SQL을 실행시키는 객체 준비
    // => SqlSession 공장 객체로부터 SqlSession 객체를 얻는다.
    // => openSession()은 수동 커밋으로 SQL을 다루는 객체를 리턴한다. (기본이 수동 커밋임)
