...
public class Exam0110 {
  ...
  public static void main(final String[] args) throws Exception {
    ...
    final SocketConfig socketConfig = SocketConfig.custom() // custom 준비!
        .setSoTimeout(15, TimeUnit.SECONDS)//
        .setTcpNoDelay(true)//
        .build();
    // (동영)build은 팩토리 메서드
    // 객체생성과정이 복잡할때 메서드를 활용하여 만드는 것을 팩토리 메서드라고 함.
  ...
  static class HttpFileHandler implements HttpRequestHandler {
    ...
    public void handle(//
      ...
      if (!file.exists()) {
        ...
        // 클라이언트가 요청한 정적자원(파일)을
        // 디렉토리이거나 읽을 수 없다면 오류 메시지를 보낸다.
        ...
        if (path.equals("/favicon.ico")) { // 파비콘은 사진이 나오는것
