...
public class App {
  ...
  private static void loadLessonData() {
    ...
    try {
      ...
      dataScan = new Scanner(in); // 라인 단위로 읽기 위해서 스캐너를 넣어둠
      int count = 0;
  ...
  private static void saveLessonData() {
    ...
    try {
      // 파일로 데이터를 쓸 때 사용할 도구를 준비한다.
      out = new FileWriter(file);
      int count = 0;

      for (Lesson lesson : lessonList) /* lessonList 차례대로 객체를 꺼내서 Lesson 에다가 넣겠다 */ {
