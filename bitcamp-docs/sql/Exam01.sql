...
= primary key  =Alternate
 ...
 ※ 주키(primary key)는 한번 결정되면 값은 절대로 바꿀 수 없다.
   => 해결?
   => 인공키(Artificial key)사용 (주로 정수 번호를 사용함) // 임의의 키를 부여하는 것 
   => 예) 회원번호
...
> create table test1(
 ...
 /* 다음과 같이 제약 조건을 모든 컬럼 뒤에 놓을 수 있다. */
create table test1(
  no int,
  name varchar(20),
  age int,
...
- 단 반드시 primary key(ex: primary key 나 unique)여야 한다.
```
alter table test1
...
alter table test1
  add constraint unique(no);  /*  no를 unique로 지정해되 된다. - auto-increment도 자동 지정된다.*/ 
...
/* auto - increment 컬럼의 값을 생력하면 마지막 값을 증가시켜서 입력한다.*/ 
...
/*값을 삭제하더라도 auto increment 는 계혹 앞으로 증가한다. */
delete from test1 where no=103;

insert into test1(name) values('eee'); /* no=104*/

insert into test1(name) values('1234567891234567891234');
